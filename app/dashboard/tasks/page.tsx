...
export default async function TasksPage() {
  ...
  const [{ data }, { data: areasData }, { data: projectsData }, { data: goalsData }] = await Promise.all([
    supabase
      .from("tasks")
      .select(
        "id, title, description, priority, status, energy_required, estimated_minutes, deadline, scheduled_date, area_id, project_id, goal_id, lever, impact_score, effort, execution_mode, device_required"
      )
  ...
  return (
    <main className="flex-1 px-6 md:px-8 py-6 max-w-3xl w-full mx-auto">
      <div className="flex items-center justify-between mb-6">
        ...
        <CreateTaskButton areas={areas} projects={projects} goals={goals} />
      </div>
      ...
      <Card>
        {tasks.length === 0 ? (
          <EmptyState
            ...
            action={<CreateTaskButton areas={areas} projects={projects} goals={goals} />}
          />
        ) : (
            ...
            {tasks.map((t) => (
              <li key={t.id} className="flex items-center justify-between gap-3 px-5 py-3">
                ...
                <div className="flex items-center gap-2.5 flex-none">
                  ...
                  <EditTaskButton task={t} areas={areas} projects={projects} goals={goals} />
