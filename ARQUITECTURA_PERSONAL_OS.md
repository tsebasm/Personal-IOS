...
| **9 · Analytics / Feedback** ✅ 2026-09-24 (`lib/engine/rollup.ts` + `lib/data/analytics.ts`; cierre semanal con snapshot inmutable en `reviews`; Insights real con tendencia de 8 semanas; "¿Qué cambió desde ayer?" en Hoy) | `weekly-rollup.ts`, snapshot en `reviews`, "¿qué cambió desde ayer/esta semana?", Insights real | Cada semana cierra con varianza + bottleneck + ajuste guardados |
| **10 · Tests / CI** ✅ 2026-09-24 (109 tests; CI: typecheck + lint + tests + build; guardas: RLS en toda tabla nueva y banner de migraciones completo. Pendiente del usuario: branch protection en GitHub para que CI bloquee merges) | Endurecimiento: cobertura de todos los motores, tests de integración de acciones clave | CI bloquea merges en rojo |
